...
export interface TranslationDict {
  ...
  // Added dynamic simulation keys
  noAckTrue: string
  noAckFalse: string
  dlxEnabled: string
  dlxDisabled: string
  stdoutAllStreamsTab: string
  stdoutDaemonListening: string
  stdoutUtfStream: string
  quickAddUsernameLabel: string
  directQueueTransFallback: string
  brokerHeaderArgs: string
  brokerRetryAttempts: string
  brokerOf: string
  brokerRequeueTooltip: string
  requeueBtnText: string
  brokerAckTooltip: string
  brokerNackTooltip: string
}
...
export const translations: Record<Language, TranslationDict> = {
  en: {
    ...
    heroDesc:
      'A professionally architected, SOLID-compliant backend using NestJS, decoupled via RabbitMQ direct queue brokers, and delivering telemetry streams to the Telegram API. Configure, run, and step-debug the entire pipeline inside this interactive panel.',
    runSignalBtn: 'Run Signal Simulation',
    browseCodeBtn: 'Browse Code Workspace',

    botDialogTitle: 'Telegram API Real Dispatcher',
    botSecureLabel:
      'Secure Storage: Your Bot API Token is stored strictly local in your web browser (localStorage). It is never sent to any external server except directly to the official api.telegram.org webhook!',
    ...
    // Added dynamic simulation keys
    noAckTrue: 'noAck: true • fast deliver',
    noAckFalse: 'noAck: false • manual status safety',
    dlxEnabled: 'Redirection to (DLX) Dead-Letter Queue Enabled',
    dlxDisabled: 'Queue Healthy (Clean stream flow)',
    stdoutAllStreamsTab: 'All Streams',
    stdoutDaemonListening: 'nest-cli daemon listening',
    stdoutUtfStream: 'UTF-8 stream on active container console',
    quickAddUsernameLabel: 'Username',
    directQueueTransFallback: 'Direct Queue Transaction',
    brokerHeaderArgs: 'Header args:',
    brokerRetryAttempts: 'Automatic MQ Retry Attempts:',
    brokerOf: 'of',
    brokerRequeueTooltip: 'Resubmit envelope back to queue (amqp.publish)',
    requeueBtnText: 'Requeue',
    brokerAckTooltip: 'Dangers: issues clean amq.ack thread response',
    brokerNackTooltip: 'Sends amq.nack',
  },
  ru: {
    ...
    heroDesc:
      'Профессионально спроектированная, SOLID-ориентированная архитектура с использованием NestJS, разделенная через брокер сообщений RabbitMQ с прямой очередью (Direct Queue) и отправкой событий телеметрии в Telegram Bot API. Запускайте сценарии и просматривайте пошаговые логи прямо в реальном времени.',
    ...
    botSecureLabel:
      '🔒 Безопасное хранилище: Ваш токен бот-API сохраняется исключительно локально в браузере (localStorage). Он никогда не передается третьим лицам и используется только для прямой отправки запросов на api.telegram.org!',
    ...
    virtualPhoneDesc:
      'При успешной обработке в nest-consumer, событие триггерит отправку отладочных данных в Telegram Bot. По умолчанию сообщения доставляются на симулируемый смартфон справа. Чтобы получать их на свой физический телефон, наберите Настроить Telegram-бота вверху и введите настоящие ключи!',
    ...
    codeIdeHeaderDesc:
      'Дерево файлов полностью иллюстрирует каноническую структуру NestJS-сервисов с соблюдением принципов абстракции и инверсии зависимостей. Кликните на файл, чтобы просмотреть исходный код.',
    ...
    // Added dynamic simulation keys
    noAckTrue: 'noAck: true • быстрая доставка',
    noAckFalse: 'noAck: false • ручное подтверждение',
    dlxEnabled: 'Перенаправление в очередь бракованных сообщений (DLX) включено',
    dlxDisabled: 'Очередь здорова (Чистый поток транзакций)',
    stdoutAllStreamsTab: 'Все потоки',
    stdoutDaemonListening: 'активен фоновый демон nest-cli',
    stdoutUtfStream: 'поток UTF-8 в активной консоли контейнера',
    quickAddUsernameLabel: 'Имя (Username)',
    directQueueTransFallback: 'Операция прямой очереди',
    brokerHeaderArgs: 'Аргументы заголовка:',
    brokerRetryAttempts: 'Автоповторы RabbitMQ MQ:',
    brokerOf: 'из',
    brokerRequeueTooltip: 'Вернуть конверт обратно в очередь (amqp.publish)',
    requeueBtnText: 'Перезапустить',
    brokerAckTooltip: 'Опасно: отправляет подтверждение amq.ack для изъятия',
    brokerNackTooltip: 'Отклонить: отправляет amq.nack',
  },
}
